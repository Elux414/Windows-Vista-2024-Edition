...
////////////////////////////////////////////////////////////////////////////////
var BIDI = "";
var L_VIEWCONTACTDETAILS_text = "Просмотр контактных сведений";
var L_BEGINSEARCH_text = "Введите текст для поиска в текущем отображении";
var L_SENDEMAIL_text = "Щелкните, чтобы отправить сообщение  ABC@MICROSOFT.COM";
...
function addTableRow(table,rowType,phoneNumber)
{
    ...
    if (imageSource != "")
    {
        ...
        oSpan.className = "phone";
        oSpan.title = phoneNumber;
...
function reFilter()
{
    ...
    function _f(o)
    {
        ...
        {           
            ...
            if( i++&1 )
            {
                oQ.style.backgroundColor = "#d6e0fd";
            }
            oQ.onmouseover = function() {omo(this)}
            oQ.onkeydown = function() {processKey(this)}
            oQ.onclick = function() {getDetailView(this)}
            oQ.innerText = s;
            if( re.source )
            {
                oQ.innerText = "";
                ...
                var oStr = new Array();
                oStr[0] = s.substring(0,index);
                oStr[1] = s.substring(index, index + tFilter.value.length);
                oStr[2] = s.substring(index + tFilter.value.length);
                var oCite = document.createElement("CITE");
                oCite.innerText = oStr[1];
                oCite.style.fontWeight = 700;
                oCite.style.fontStyle = "normal";
                oCite.style.backgroundColor = "#99B0DD";
                oQ.insertAdjacentText("afterBegin",oStr[0]);
                oQ.appendChild(oCite);
                oQ.insertAdjacentText("beforeEnd",oStr[2]);
            }   
            hList.appendChild(oQ);
