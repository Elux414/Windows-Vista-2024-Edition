...
6. WindowsExtendedBlur - простая программа для включения эффекта прозрачности с размытием в "Блокноте". Данную программу следует поставить в автозапуск, легче всего это делается через планировщик задач. В нём необходимо создать простую задачу, триггером назначить событие включения компьютера и указать путь до исполняемого файла.
